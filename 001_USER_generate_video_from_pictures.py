...
async def process_user_images_parallel(
    converter: FalImageToVideo,
    image_dir: str = "./user_images",
    pattern: str = "*.jpg",
    model: str = "fal-ai/veo2/image-to-video",
    ...
    """
    Process multiple user_images in parallel

    Args:
        converter: FalImageToVideo instance
        image_dir: Directory containing user_images
    ...
    # Get all user_images matching the pattern
    image_paths = glob.glob(os.path.join(image_dir, pattern))

    if not image_paths:
        print(
            f"No user_images found matching pattern {pattern} in directory {image_dir}"
        )
        return []

    # Sort user_images by modification time (newest first)
    image_paths.sort(key=lambda x: os.path.getmtime(x), reverse=True)

    print(f"Found {len(image_paths)} user_images to process")
...
async def main_async():
    """Asynchronous main function to process user_images in parallel"""
    import os
    ...
    # Process all JPG/JPEG user_images in the current directory
    patterns = ["*.jpg", "*.jpeg", "*.JPG", "*.JPEG"]
    all_results = []

    for pattern in patterns:
        results = await process_user_images_parallel(
            converter=converter,
            image_dir="./user_images",
            pattern=pattern,
            model="fal-ai/veo2/image-to-video",
            prompt="Generate a smooth, natural motion video",
            max_concurrent=3,  # Process 3 user_images at a time
        )
        all_results.extend(results)

    # Print a summary of results
    print("\n=== Processing Summary ===")
    print(f"Total user_images processed: {len(all_results)}")
